...
db.connect().then(() => {
    ...
    db.query('CREATE TABLE IF NOT EXISTS notes (username VARCHAR(252) REFERENCES users(username), tag VARCHAR(252) PRIMARY KEY, content VARCHAR(4096), x INTEGER, y INTEGER, width INTEGER, height INTEGER, zindex INTEGER, colors VARCHAR(512))');
...
function addNote(req, res) {
    ...
    var values = [username, input.tag, input.content, input.x, input.y, input.width, input.height, input.zindex, input.colors]
...
function updateNote(req, res){
    ...
    var arr = [input.newcontent, input.newx, input.newy, input.newW, input.newH, input.newZ, input.newColors, username, input.tag]
    db.query("UPDATE notes SET content=$1, x=$2, y=$3, width=$4, height=$5, zindex=$6, colors=$7 WHERE username=$8 AND tag=$9", arr).then(() => {
...
function getNotes(req, res) {
    ...
    db.query("SELECT tag, content, x, y, width, height, zindex, colors FROM notes WHERE username=$1", [username], function(error, resp){
        // send the array
